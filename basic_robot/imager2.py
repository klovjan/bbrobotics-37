...
class Imager:
    ...
    def __init__(self, fid=None, image=None, width=100, height=100, background='black', mode='RGB'):
        self.fid = fid  # The image file
        self.image = image  # A PIL image object
        self.xmax, self.ymax = width, height  # These can change if there's an input image or file
        self.mode = mode
    ...
    def copy_image_dims(self, im2):
        im2.xmax, im2.ymax = self.xmax, self.ymax
    ...
    def resize(self, new_width, new_height, image=None):
    ...
    def map_image(self, func, image=None):
    ...
    def map_image2(self, func, image=None):
    ...
    def map_color_wta(self, image=None, thresh=0.34):
        image = image if image else self.image
    ...
    def gen_grayscale(self, image=None): return self.scale_colors(image=image, degree=0)

    def scale_colors(self, image=None, degree=0.5):
        image = image if image else self.image
    ...
    def concat_vert(self, im2=None, background='black'):
    ...
    def concat_horiz(self, im2=None, background='black'):
        im2 = im2 if im2 else self  # concat with yourself if no other imager is given.
        im3 = Imager()
...
def ptest1(fid1='images/kdfinger.jpeg', fid2="images/einstein.jpeg", steps=5, newsize=250):
    im1, im2 = Imager(fid1), Imager(fid2)
    im1, im2 = im1.resize(newsize, newsize), im2.resize(newsize, newsize)
